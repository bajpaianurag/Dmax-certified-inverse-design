...
from sklearn.metrics import (
    r2_score, mean_absolute_error, mean_squared_error, mean_pinball_loss
)
...
ALPHA = 0.15          # target 85% coverage
ROBUST_EPS = 0.02     # set to 0.02 for a clearer ±1 at.% interpretation
ROBUST_SAMPLES = 64   # Monte Carlo jitters per composition for robust CP
QT_TAU = 0.95         # high-tail quantile for discovery

THRESHOLDS_MM = tuple(sorted({2.0, 4.0, 5.0, 7.0, 10.0, 15.0, 20.0}))

MIN_GROUP_N     = 30
FAMILY_MIN_TEST = 10

if not Path(RAW_CSV).exists():
    raise FileNotFoundError(f"Could not find input dataset: {RAW_CSV}")

if not (0.0 < ALPHA < 1.0):
    raise ValueError(f"ALPHA must be in (0,1); got {ALPHA}")

if not (0.5 <= QT_TAU < 1.0):
    raise ValueError(f"QT_TAU should be high-tail (>=0.5) and <1; got {QT_TAU}")

if ROBUST_SAMPLES < 8:
    warnings.warn("ROBUST_SAMPLES is very low; robust quantiles may be noisy.", RuntimeWarning)

if not (0.0 <= ROBUST_EPS <= 0.25):
    warnings.warn("ROBUST_EPS unusually large; bounds may become overly conservative.", RuntimeWarning)
...
def jitter_in_L1_ball_simplex(x, eps, K, rng, include_x=True):
    """
    Sample K compositions within an L1-ball of radius `eps` around x, on the simplex.
    Implementation: draw y ~ Dirichlet(1), then use x' = (1 - t)*x + t*y with t = min(1, eps / ||y-x||_1).

    IMPORTANT (at.% interpretation):
    - The L1 distance equals twice the total mass moved between components.
      So eps = 0.02 corresponds to moving ~1 at.% from some elements to others.
      (eps = 0.04 → ~2 at.% swap, etc.)

    Args
    ----
    x : 1D array of base composition (fractions summing to ~1)
    eps : L1 radius (fraction units)
    K : number of jitters
    rng : numpy.random.Generator
    include_x : if True, ensure the base `x` is included among the returned samples (helps stability)

    Returns
    -------
    samples : (K, d) array, each row on the simplex and within L1 <= eps of x
    """
    ...
    n_draws = max(K - int(include_x), 0)
    if n_draws > 0:
        t_levels = (np.arange(n_draws) + 0.5) / n_draws
        for t0 in t_levels:
            ...
            t = min(1.0, eps / l1) * t0
            xj = (1.0 - t) * x + t * y
            xj = np.clip(xj, 0.0, None)
            s = xj.sum()
            xj = xj / (s if s > 0 else 1.0)
            samples.append(xj)

    samples = np.array(samples, float)
    if samples.shape[0] < K:
        for _ in range(K - samples.shape[0]):
            y = rng.dirichlet(np.ones(d))
            l1 = float(np.sum(np.abs(y - x)))
            if l1 < 1e-15:
                samples = np.vstack([samples, x[None, :]])
            else:
                t = min(1.0, eps / l1)
                xj = (1.0 - t) * x + t * y
                s = xj.sum()
                xj = np.clip(xj, 0.0, None) / (s if s > 0 else 1.0)
                samples = np.vstack([samples, xj[None, :]])
    elif samples.shape[0] > K:
        samples = samples[:K]
...
# Composition signature & duplicates
GRID_DECIMALS = 3
GRID_SCALE    = 10 ** GRID_DECIMALS

def mean_abs_dev_mean(s):
    """Mean absolute deviation about the mean (legacy pandas .mad())."""
    s = np.asarray(s, float)
    m = np.nanmean(s)
    return float(np.nanmean(np.abs(s - m)))

def mad_about_median(s):
    """Median absolute deviation about the median (robust)."""
    s = np.asarray(s, float)
    med = np.nanmedian(s)
...
def composition_signature(df, elem_cols, decimals=GRID_DECIMALS):
    """
    Human-readable signature at a fixed rounding grid (e.g., 0.1 at.%).
    Uses the *column order of elem_cols*; upstream we already fixed this to PT order.
    """
...
def composition_signature_int(df, elem_cols, decimals=GRID_DECIMALS):
    """
    Integer signature vector at the same grid; useful for hashing, joins, or sanity checks.
    """
    comp = df[elem_cols].to_numpy(float)
    comp = np.clip(comp, 0.0, None)
...
def composition_signature_hash(sig_int, algo="md5"):
    """
    Compact md5 hash from the integer signature vector.
    """
    import hashlib
    hashes = []
    for row in sig_int:
...
def heaping_report_plus(y, tol=5e-3):
    """
    Enhanced heaping audit (JSON-safe).
    Adds:
      - quarter-fraction heaping rates (0, .25, .5, .75 ± tol)
      - integer heaping rate
      - first-decimal digit preference table + chi-square statistic
      - fractional-part histogram (10 bins)
      - step-size mode estimate (unchanged, but guarded)
    """
...
def savefig_both(path_png):
    """Save PNG, PDF, and SVG with tight bbox; suitable for manuscript."""
...
def annotate_bars(ax, fmt="{:d}"):
    """Annotate bar tops with integer counts; skip non-finite/zero heights."""
    for p in ax.patches:
        h = p.get_height()
...
def add_percent_axis_top(ax, total):
    """Add a top x-axis showing percent of dataset for a count-based bar chart."""
    if total <= 0:
        return
    def count_to_pct(x):
...
def freedman_diaconis_bins(a):
    """Adaptive bin count for histograms (handles small-N gracefully)."""
    a = np.asarray(a, float)
    a = a[np.isfinite(a)]
    n = a.size
    if n < 2:
...
print("Saved enhanced EDA figures to:", FIGDIR)
...
def balanced_assign_groups(sig_to_indices, targets, seed=SEED, universe_index=None):
    """
    Assign signature groups to splits to match target sizes as closely as possible.

    Parameters
    ----------
    sig_to_indices : dict[str, np.ndarray]
        Mapping signature -> array of *global* row indices (as returned by df.groupby(...).indices).
    targets : dict[str, int]
        Desired row counts per split.
    seed : int
        RNG seed for tie-breaking.
    universe_index : array-like or None
        The set of row indices that must be exactly partitioned. If None, it is inferred
        as the union of all values in sig_to_indices.
    """
...
# Assert full coverage of the working element set early
_missing = {
...
# Helper to produce a NumPy vector aligned to elem_cols
...
# Precompute aligned vectors once
...
# Freeze the exact training schema
...
def make_features_from_compositions(X_comp):
    """
    Build inference-time features with EXACTLY the training schema (X_COLUMNS).

    Parameters
    ----------
    X_comp : array-like, shape (n_samples, len(elem_cols)) or (len(elem_cols),)
        Compositions as fractions (rows should sum ~ 1). Negatives are clipped to 0
        and rows are renormalized safely.

    Returns
    -------
    feats_full : pandas.DataFrame, shape (n_samples, len(X_COLUMNS))
        Feature matrix aligned to the training schema, including fam_* dummies
        over the frozen vocabulary FAM_LEVELS.
    """
...
ax.text(0.02, 0.98, f"RMSE={rmse_mm:.2f} mm\nMAE={mae_mm:.2f} mm",
...
# Error vs composition entropy (normalized)
...
# HPO traces (sanity & transparency)
# ET
try:
    et_cv = pd.read_csv(HPO_DIR / "et_bayes_cv_results.csv")
    plt.figure(figsize=(6.2, 3.8))
    # BayesSearchCV doesn't always expose iteration; we use rank or index
    ax = plt.gca()
...
try:
    ...
    print("CatBoost HPO trace skip:", e)
...
# Multi-τ Sweep (0.80–0.98)
SWEEP_DIR = OUTDIR / "source_data"
SWEEP_DIR.mkdir(parents=True, exist_ok=True)

taus = np.round(np.arange(0.80, 0.981, 0.02), 2)
...
# Plots: τ vs observed coverage and pinball
...
# CatBoost high-τ (fixed loss; we search architecture & regularization only)
...
S_cal_hi_rob = robust_scores_lower(
    y_true_log=y_cal,
    model=cat_qt_hi,
    X_elem=X_cal_elem,
    eps=ROBUST_EPS,
...
q_robust_hi = weighted_quantile(S_cal_hi_rob, 1 - ALPHA)

if not np.isfinite(q_robust_hi):
    q_robust_hi = weighted_quantile(S_cal_hi, 1 - ALPHA)
...
for a in alpha_grid:
    ...
    S_rob_a = S_cal_hi_rob
    q_rob_a = weighted_quantile(S_rob_a, 1 - a)
    ...
    rows_alpha.append({"alpha": float(a), "target_coverage": float(1 - a),
                       "coverage_marginal": cov_marg_a, "coverage_robust": cov_rob_a})
...
rows_eps = []
for eps in eps_grid:
    S_eps = robust_scores_lower(y_cal, cat_qt_hi, X_cal_elem, eps=eps, K=ROBUST_SAMPLES, rng=rng)
...
per_test = pd.DataFrame({
    ...
    "L_robust_mm": L_robust_hi_mm_test,
    "covered_marginal": (y_test_mm >= L_marginal_hi_mm).astype(int),
    "covered_robust": (y_test_mm >= L_robust_hi_mm_test).astype(int),
})
...
# Visualization for τ=0.98 quantile + (marginal/robust) conformal
FIGDIR = OUTDIR / "reports" / "figures"
FIGDIR.mkdir(parents=True, exist_ok=True)
SRC_DIR = OUTDIR / "source_data"
SRC_DIR.mkdir(parents=True, exist_ok=True)
...
# Sharpness: gap = exp(q̂τ) − L (smaller is better)
...
for xv, yv, nn in zip(df_rel_plot["bin_center"], df_rel_plot["coverage"], df_rel_plot["n"]):
    ax.text(xv, min(0.98, yv + 0.03), f"n={nn}", ha="center", fontsize=9)
ax.axhline(1-ALPHA, ls="--", label=f"Target {1-ALPHA:.2f}")
...
for i, v in enumerate(df_fam_cov["coverage_robust"]):
    ax.text(min(0.98, v), i, f" {v:.2f}", va="center")
ax.legend(frameon=False, loc="lower right")
...
try:
    df_alpha = pd.read_csv(SRC_DIR / "coverage_vs_alpha.csv")
    ...
    for a in alpha_grid:
        ...
        q_rob_a = weighted_quantile(S_cal_hi_rob, 1 - a)
        L_rob_a = robust_L_for_comps_hi(X_elem_test, q_cal_robust=q_rob_a, model=cat_qt_hi,
                                        eps=ROBUST_EPS, K=ROBUST_SAMPLES, rng=_rng_plot)
        cov_rob_a = float(np.mean(y_test_mm >= L_rob_a))
        rows_alpha.append({"alpha": a, "target_coverage": 1 - a,
                           "coverage_marginal": cov_marg_a, "coverage_robust": cov_rob_a})
    df_alpha = pd.DataFrame(rows_alpha)

plt.figure(figsize=(6.4, 4.0))
sns.lineplot(x="alpha", y="coverage_marginal", data=df_alpha, marker="o", label="Marginal")
sns.lineplot(x="alpha", y="coverage_robust", data=df_alpha, marker="o", label="Robust")
sns.lineplot(x="alpha", y="target_coverage", data=df_alpha, linestyle="--", label="Target")
plt.gca().invert_xaxis()
plt.ylim(0, 1); plt.ylabel("Empirical coverage"); plt.xlabel("α")
plt.title("Coverage vs α (test)")
plt.legend(frameon=False)
savefig_both(FIGDIR / "hi_tau_coverage_vs_alpha.png")

# Coverage vs epsilon plot
try:
    df_eps = pd.read_csv(SRC_DIR / "coverage_vs_epsilon.csv")
except FileNotFoundError:
    eps_grid = [0.000, 0.005, 0.010, 0.020, 0.030]
    rows_eps = []
    for eps in eps_grid:
        S_eps = robust_scores_lower(y_cal, cat_qt_hi, df.loc[idx_cal, elem_cols].to_numpy(),
                                    eps=eps, K=ROBUST_SAMPLES, rng=_rng_plot)
        S_eps = np.maximum(0.0, np.nan_to_num(S_eps, nan=0.0))
        q_eps = weighted_quantile(S_eps, 1 - ALPHA)
        L_eps = robust_L_for_comps_hi(X_elem_test, q_cal_robust=q_eps, model=cat_qt_hi,
                                      eps=eps, K=ROBUST_SAMPLES, rng=_rng_plot)
        cov_eps = float(np.mean(y_test_mm >= L_eps))
        rows_eps.append({"eps": eps, "q_robust": q_eps, "coverage_robust": cov_eps})
    df_eps = pd.DataFrame(rows_eps)

plt.figure(figsize=(6.4, 4.0))
sns.lineplot(x="eps", y="coverage_robust", data=df_eps, marker="o", label="Robust coverage")
plt.axhline(1-ALPHA, ls="--", label=f"Target {1-ALPHA:.2f}")
plt.xlabel("ε (L1 on simplex; fraction)"); plt.ylabel("Coverage (test)")
plt.title("Coverage vs robustness radius ε")
plt.legend(frameon=False)
savefig_both(FIGDIR / "hi_tau_coverage_vs_epsilon.png")
...
def robust_min_logq_batch(q_model, X_elem, eps, K, rng=None, chunk=4096):
    ...
    def _flush():
        nonlocal qmin, buf_X, buf_id, buf_count
        if not buf_X:
            return
        Xj = np.vstack(buf_X)
        feats = make_features_from_compositions(Xj) 
        preds = np.asarray(q_model.predict(feats), float)
        assert preds.shape[0] == buf_count, "Batch size mismatch in robust_min_logq_batch."
        start = 0
...
# Faster, memory-safe robust prediction of min log-quantile on TEST
def robust_min_logq_batch(q_model, X_elem, eps, K, rng=None, chunk=4096):
    """
    For each x in X_elem, jitter K times (L1-ball radius eps), predict log-quantiles,
    and return the per-sample minimum (worst-case) log q_τ.
...
q_robust_hi = weighted_quantile(S_cal_rob, 1 - ALPHA)
if not np.isfinite(q_robust_hi):
    S_cal_marg = np.maximum(0.0, (cat_qt_hi.predict(X_cal) - y_cal))
    q_robust_hi = weighted_quantile(S_cal_marg, 1 - ALPHA)
if not np.isfinite(q_robust_hi):
    raise RuntimeError("q_robust_hi is not finite after fallbacks; check scores and features.")

# Robust TEST bounds (mm)
qmin_test_log = robust_min_logq_batch(
    cat_qt_hi, X_test_elem, eps=ROBUST_EPS, K=ROBUST_SAMPLES,
    rng=np.random.default_rng(SEED + 777), chunk=4096
)
L_robust_mm = np.exp(qmin_test_log - q_robust_hi)

# Ensure qhat_test_hi exist even if prior cells didn't run
...
if "L_robust_mm" in globals():
    robust_label = f"Robust (±{ROBUST_EPS*100:.0f} at.%)"
...
# Inverse design — tree surrogate + two-stage robustness (diagnostics + adaptive gates)
...
# prediction-time gate for qτ (mm) used to pre-filter BO pool (can be turned off or made adaptive below)
if 'DSTAR_PRED_MM' not in globals():
    DSTAR_PRED_MM = 4.0

# dataset compositions matrix (if not already defined upstream)
...
if "STAGE1_ROBUST_SAMPLES" not in globals(): STAGE1_ROBUST_SAMPLES = 64
if "STAGE1_MC_REPS"        not in globals(): STAGE1_MC_REPS        = 1

# Use your original robust settings for Stage-2 (defaults to existing globals)
if "STAGE2_ROBUST_SAMPLES" not in globals(): STAGE2_ROBUST_SAMPLES = ROBUST_SAMPLES
if "STAGE2_MC_REPS"        not in globals(): STAGE2_MC_REPS        = 3
if "STAGE2_RESCORE_TOP"    not in globals(): STAGE2_RESCORE_TOP    = 200

# Choose your allowed element list (order matters)
allowed_elems = ["Ag","Al","Be","Co","Cu","Fe","Hf","La","Nb","Ni","Ti","Zr"]
...
class CertifiedObjective:
    """
    Robust certified lower bound L_robust(x) with:
      - CRN seeds for smoother optimization.
      - Optional M replicate seeds (MC) → mean ± SE.
      - Dict cache across search iterations.
    """
    def __init__(self, model, q_cal_robust, eps=ROBUST_EPS, K=ROBUST_SAMPLES,
        ...
        self.crn_dirs = [self.crn.integers(0, 10**9) for _ in range(10**5)]
        self._cache = {}
    ...
    def __call__(self, x_allowed):
        ...
        seeds = [self.crn_dirs[len(self._cache) % len(self.crn_dirs)]]
        for r in range(self.mc_reps - 1):
            seeds.append(SEED + 1234 + r*9973)
...
def robust_sweep_for_row(row, eps_list=(0.000, 0.005, 0.010, 0.020, 0.030),
...
if "NOVELTY_L1_ATPCT" not in globals():
    NOVELTY_L1_ATPCT = 2.0  # default novelty gate (at.% L1 to ref set)

def propose_candidates_BO(
    obj: CertifiedObjective,
    n_calls=3000, n_random_starts=40, xi=0.02, random_state=SEED,
    diversity_batch=30, diversity_min_L1_atpct=1.0,
    novelty_min_L1_atpct=NOVELTY_L1_ATPCT,
    novelty_reference="train",       # "train", "all", or None
    hard_caps=None, max_elements=None,
    stage2_rescore_top=STAGE2_ROBUST_SAMPLES,
    ...
    _trace.to_csv(OUTDIR / "source_data" / "bo_trace.csv", index=False)
    ...
    # "max possible" optimistic predictor (τ=0.98 if available)
    if ("cat_qt_grid" in globals()) and isinstance(cat_qt_grid, dict) and (0.98 in cat_qt_grid):
        q98_log     = np.asarray(cat_qt_grid[0.98].predict(feats0), float)
        pred_max_mm = np.exp(q98_log)
    else:
    ...
    pool_all.to_csv(designed_dir / "bo_tried_all.csv", index=False)

    # Exceedance snapshot BEFORE filtering (preregistered)
    try:
        BENCHMARK_MM = float(np.nanmax(np.exp(y_log)))  # y_log is ln(mm)
    except Exception:
        # optional: try to read dataset max from a saved table; else fall back
        try:
    ...
    # save a preregistered, prefilter snapshot for SI/reproducibility
    (designed_dir).mkdir(parents=True, exist_ok=True)
    pool_all.assign(BENCHMARK_MM=float(BENCHMARK_MM)).to_csv(
        designed_dir / "bo_pool_pre_filter_with_exceedance.csv", index=False
    )

    # ---------- Prefilter diagnostics ----------
    n0 = len(pool_all)
    # adaptive gate (if requested) overrides fixed gate
    ...
    print(f"  novelty at.%:   min={np.nanmin(min_L1_atpct):.2f}, p50={np.nanmedian(min_L1_atpct):.2f}, p90={np.nanpercentile(min_L1_atpct,90):.2f}")
    ...
    pool_prefilter.to_csv(designed_dir / "bo_prefiltered.csv", index=False)

    # Graceful fallback: if empty, relax prediction gate once (keep novelty)
    if len(pool_prefilter) == 0:
        print("[BO prefilter] 0 survivors → relaxing qτ gate once (pred_gate_mm=None).")
    ...
    if len(exceed_prefilter):
        ...
        exceed_prefilter.to_csv(designed_dir / f"advanced_candidates_pred_ge_{int(BENCHMARK_MM)}mm_all_prethin.csv", index=False)
...
# --- add CIs BEFORE any thresholding ---
...
# --- mark novelty vs entire dataset (rounded signature) ---
ROUND_TOL_ATPCT = 0.5  # treat comps equal within 0.5 at.% as duplicates
...
for _, r in df.iterrows():
    ...
    for e in allowed_elems_present:
        if f"atpct_{e}" in r:
            ...
            vals.append(float(r[f"at_{e}"]))          # already at.%
        elif e in r:
            vals.append(100.0 * float(r[e]))          # fraction → at.%
        else:
            vals.append(0.0)
...
# Exceeders (final, with NOVEL labels)
try:
...
if pref_csv.exists():
    ...
    # Add rounded-signature & novelty vs ALL (same tolerance you used above)
...
# --- ε-sensitivity exports for paper/SI (NEW) ---
# Choose which pool to analyze; ET by default if both exist
_pool_for_eps = bo_df_et if "bo_df_et" in globals() else bo_df
...
df_eps.to_csv(_rep / "eps_sensitivity_top30.csv", index=False)
...
# Predict multi-τ (in mm) and add tail slope Δq = q0.98 - q0.90
for t in TAUS:
    pred_log = np.asarray(cat_qt_grid[t].predict(feats_cand), float)
    cand_df[f"q{int(round(100*t))}_mm"] = np.exp(pred_log)

if 0.98 in cat_qt_grid and 0.90 in cat_qt_grid:
    cand_df["tail_slope_mm"] = cand_df["q98_mm"] - cand_df["q90_mm"]
...
def _key_from_x(x, eps, K, nd=4):
    # round at.% to avoid microscopic duplicates; include K & eps
...
def eval_robust_L_allowed(x_allowed, eps=ROBUST_EPS, K=ROBUST_SAMPLES, rng=None):
    """Certified robust lower bound on the allowed simplex; returns L_robust in mm."""
    if rng is None:
    ...
    Xj_allowed = jitter_allowed_simplex(x_allowed, eps=eps, K=K, rng=rng)
    Xj_full = embed_allowed_to_full(Xj_allowed)
    feats = make_features_from_compositions(Xj_full)
...
def tornado_data_for_candidate(x_allowed, step_atpct=1.0):
    ...
    for e in range(d):
        ...
        if x0[e] > step:
            for i in range(d):
                ...
                s = x.sum(); x = x / s if s > 0 else x
                val = eval_robust_L_allowed(x)
                worst_drop = min(worst_drop, val - base)  # most negative
    ...
    return df_tor  # <-- IMPORTANT: return!
...
def plot_tornado(df_tor, title, save_path):
    ...
    plt.barh(y, -dec, left=0,   alpha=0.75, label="−1 at.% from element")  # left (negative)
    plt.barh(y,  inc, left=0,   alpha=0.75, label="+1 at.% to element")    # right (positive)
...
# Certified region maps on ternary slices
def barycentric_grid(n=41):
    """Return integer triplets (i,j,k) with i+j+k=n-1 mapped to fractions in the 2-simplex."""
    pts = []
    for i in range(n):
        for j in range(n - i):
            k = (n - 1) - i - j
...
def eval_ternary_slice(elems3, Dstar=4.0, n=41, eps=ROBUST_EPS, K=ROBUST_SAMPLES):
    """Evaluate L_robust on a 3-element simplex slice; others set to zero."""
    assert len(elems3) == 3
    idx3 = [elem_cols.index(e) for e in elems3]
    B = barycentric_grid(n=n)  # (M,3)
    M = B.shape[0]
    X_full = np.zeros((M, len(elem_cols)), float)
    X_full[:, idx3] = B
    vals = []
    rng = np.random.default_rng(SEED+888)
    for b in B:
        Xj_sub = jitter_in_L1_ball_simplex(b, eps=eps, K=K, rng=rng)
        Xj = np.zeros((K, len(elem_cols)), float)
        Xj[:, idx3] = Xj_sub
        feats = make_features_from_compositions(Xj)
        qj = np.asarray(cat_qt_hi.predict(feats), float)
        vals.append(float(np.exp(np.min(qj) - q_robust_hi)))
    vals = np.array(vals, float)
    return B, vals
    
def _ternary_to_xy(B):
    """Map barycentric (a,b,c) with a+b+c=1 to 2D coords for equilateral triangle plotting."""
    a, b, c = B[:,0], B[:,1], B[:,2]
    x = 0.5*(2*b + c)
    y = (np.sqrt(3)/2.0)*c
    return x, y

def plot_certified_region(elems3, Dstar=4.0, n=41, eps=ROBUST_EPS, K=ROBUST_SAMPLES):
    B, vals = eval_ternary_slice(elems3, Dstar=Dstar, n=n, eps=eps, K=K)
    x, y = _ternary_to_xy(B)
    passed = vals >= Dstar

    plt.figure(figsize=(6.0,5.6))
    sc = plt.scatter(x, y, c=vals, s=18)
    plt.tricontour(x, y, vals, levels=[Dstar], colors=["k"], linewidths=1.2)
    cb = plt.colorbar(sc); cb.set_label("Certified L_robust (mm)")
    plt.title(f"Certified region (≥{Dstar:.0f} mm) on ternary: {elems3[0]}–{elems3[1]}–{elems3[2]}")
    plt.axis("equal"); plt.axis("off")
    plt.tight_layout()
    fname = f"ternary_cert_region_{elems3[0]}_{elems3[1]}_{elems3[2]}_{int(Dstar)}mm"
    plt.savefig(FIGDIR / f"{fname}.png", dpi=300, bbox_inches="tight")
    plt.savefig(FIGDIR / f"{fname}.pdf", bbox_inches="tight")
    plt.close()
    ...
    df_src = pd.DataFrame({
        ...
        elems3[2]: B[:,2]*100.0,
        "L_robust_mm": vals,
        f"pass_ge_{int(Dstar)}mm": passed.astype(int)
    })
    df_src.to_csv(SRC_DIR / f"{fname}.csv", index=False)

if len(cand_df):
    row0 = cand_df.sort_values("L_robust_mm", ascending=False).iloc[0]
    # try to read atpct_*, else reconstruct from fractions
    atpcts = []
    for e in allowed_elems_present:
        if f"atpct_{e}" in row0: atpcts.append((e, float(row0[f"atpct_{e}"])))
        elif f"frac_{e}" in row0: atpcts.append((e, 100.0*float(row0[f"frac_{e}"])))
        elif e in row0:           atpcts.append((e, 100.0*float(row0[e])))
    elems3 = [e for e, _ in sorted(atpcts, key=lambda kv: -kv[1])[:3]]
    plot_certified_region(elems3, Dstar=4.0, n=41)
...
from pathlib import Path
import json, sys, os, hashlib, joblib, subprocess, platform, datetime as _dt
import numpy as np
import pandas as pd
...
run_cfg = {
    ...
    "bo_params": {
        ...
        "diversity_min_L1_atpct": _maybe(1.0),
...
# Stress curves: coverage & sharpness vs tolerance ε (with CI + CSV) — FIXED
if "savefig_bundle" not in globals():
    def savefig_bundle(stem, src_df=None, meta=None):
        path_png = FIGDIR / f"{stem}.png"
        path_pdf = FIGDIR / f"{stem}.pdf"
        ...
        if src_df is not None:
            (OUTDIR / "source_data").mkdir(parents=True, exist_ok=True)
            src_df.to_csv(OUTDIR / "source_data" / f"{stem}.csv", index=False)
        if meta is not None:
            with open(OUTDIR / "source_data" / f"{stem}_meta.json", "w", encoding="utf-8") as f:
                json.dump(meta, f, indent=2)

if "bootstrap_ci_indices" not in globals():
    def bootstrap_ci_indices(fn, n, B=1000, seed=SEED+303):
        rng = np.random.default_rng(seed)
        idx = np.arange(n)
        vals = np.empty(B, float)
        for b in range(B):
...
# ---- small safety fallbacks for objects this cell uses
...
row_sums = X_test_elem_full.sum(axis=1, keepdims=True)
X_test_elem_full = np.divide(X_test_elem_full, np.where(row_sums > 0, row_sums, 1.0))
X_test_elem_sub  = X_test_elem_full[eval_subset]

y_test_sub_log   = y_test[eval_subset]
y_test_mm_sub    = np.exp(y_test_sub_log)

if "qhat_test_hi" not in globals():
    qhat_test_hi = cat_qt_hi.predict(X_test)
qhat_test_hi_sub     = np.asarray(qhat_test_hi, float)[eval_subset]
qhat_test_hi_sub_mm  = np.exp(qhat_test_hi_sub)

if "qhat_cal_hi" not in globals():
    qhat_cal_hi = cat_qt_hi.predict(X_cal)
S_cal_hi = np.maximum(0.0, np.asarray(qhat_cal_hi, float) - y_cal)
q_marginal_hi = weighted_quantile(S_cal_hi, 1 - ALPHA)

X_cal_elem = df.loc[idx_cal, elem_cols].to_numpy(float)
rs = X_cal_elem.sum(axis=1, keepdims=True)
X_cal_elem = np.divide(X_cal_elem, np.where(rs > 0, rs, 1.0))

_has_fast = "robust_min_logq_batch" in globals()

def _robust_min_logq(q_model, X_elem, eps, K, rng, chunk=4096):
    """Return min_j q̂τ(x′_j) in log space for each x, using jitter within L1-ball(ε)."""
    if _has_fast:
        # fast batch path if you defined it elsewhere
        return robust_min_logq_batch(q_model, X_elem, eps=eps, K=K, rng=rng, chunk=chunk)
    out = np.full(len(X_elem), np.inf, float)
    for i, x in enumerate(X_elem):
        Xj = jitter_in_L1_ball_simplex(x, eps=eps, K=K, rng=rng)
        rs = Xj.sum(axis=1, keepdims=True)
        Xj = np.divide(Xj, np.where(rs > 0, rs, 1.0))
        feats = make_features_from_compositions(Xj)
        # *** FIX: use the q_model passed in, not a hard-coded cat_qt_hi ***
        qj = np.asarray(q_model.predict(feats), float)
        out[i] = float(np.min(qj))
    return out

# Compute bounds on TEST subset for each ε
records = []
for eps in eps_grid:
    if eps == 0.0:
        q_eps = q_marginal_hi
        ...
        rng_cal = np.random.default_rng(SEED + int(10000 * (1 + eps)))
        # *** FIX: call with the correct signature (positional args) ***
        S_cal_eps = robust_scores_lower(
            y_cal,                 # y on log scale for CAL
            cat_qt_hi,            # quantile model
            X_cal_elem,           # CAL compositions (fractions)
            eps=eps, K=ROBUST_SAMPLES, rng=rng_cal
        )
        S_cal_eps = np.maximum(0.0, np.nan_to_num(S_cal_eps, nan=0.0))
        q_eps = weighted_quantile(S_cal_eps, 1 - ALPHA)

        rng_te = np.random.default_rng(SEED + int(20000 * (1 + eps)))
        qmin_sub_log = _robust_min_logq(cat_qt_hi, X_test_elem_sub, eps=eps, K=ROBUST_SAMPLES, rng=rng_te)
        L_eps = np.exp(qmin_sub_log - q_eps)

    m = np.isfinite(L_eps) & np.isfinite(y_test_mm_sub) & np.isfinite(qhat_test_hi_sub_mm)
    if not np.any(m):
        ...
        def cov_fn(res_idx):
            ...
            return np.mean(y_test_mm_sub[m][idx] >= L_eps[m][idx])
    ...
    # per-point export for this ε (subset)
    df_one = pd.DataFrame({
        "idx_test_local": eval_subset[m],
        "true_mm": y_test_mm_sub[m],
        "qhat_hi_mm": qhat_test_hi_sub_mm[m],
        "L_mm": L_eps[m]
    })
    df_one.to_csv(OUTDIR / "source_data" / f"stress_subset_eps_{int(100*eps):02d}.csv", index=False)

df_stress = pd.DataFrame(records)

# coverage vs ε with 95% CI + target line
plt.figure(figsize=(6.4, 4.0))
...
for Dstar in BOARD_THRESHOLDS:
    ...
    if has_novel:
        ...
        if "tail_slope_mm" in board.columns:
            ...
            cb = plt.colorbar(sc); cb.set_label("Δq = q98 − q90 (mm)")
        else:
            plt.scatter(xnov, yL)
...
figs_extra += [
    ...
    FIGDIR/"fig_tail_slope_vs_q98.png",
...
src_extra += [
    ...
    SRC_DIR / "bo_trace.csv",                   # if you wrote it inside propose_candidates_BO
    SRC_DIR / "bo_trace_from_memory.csv",       # if you exported from memory
    ...
    # If you also export per-family random test tables, include them too:
...
robust_label = f"Robust (±{ROBUST_EPS*100:.0f} at.%)" if 'ROBUST_EPS' in globals() else "Robust"
...
# Bayes relation: p_test(x)/p_cal(x) = (π_cal/π_test) * [p(test|x) / p(cal|x)]
...
if 'make_quantile_estimator' not in globals():
    def make_quantile_estimator(tau, seed=None):
        try:
            from catboost import CatBoostRegressor
            return CatBoostRegressor(
...
def robust_scores_lower(y_true_log, model, X_elem, eps, K, rng):
    ...
    for i, (y_i, x_i) in enumerate(zip(y_true_log, X_elem)):
        Xj = jitter_in_L1_ball_simplex(x_i, eps=eps, K=K, rng=rng)
        # renormalize compositions just in case
        rs = Xj.sum(axis=1, keepdims=True)
        Xj = np.divide(Xj, np.where(rs > 0, rs, 1.0))
        feats = make_features_from_compositions(Xj)
...
ROB_LABEL = f"Robust (±{ROBUST_EPS*100:.0f} at.%)"
...
for frac in FRACTIONS:
    ...
    for r in range(R_REP):
        ...
        model = make_quantile_estimator(TAU_USED, seed=SEED + 123 + r)
...
def _robust_scores_lower(y_true_log, model, X_elem, eps, K, rng):
    y_true_log = np.asarray(y_true_log, float)
    S = np.empty_like(y_true_log)
    for i, (y_i, x_i) in enumerate(zip(y_true_log, X_elem)):
        Xj = _jitter_in_L1_ball_simplex(x_i, eps=eps, K=K, rng=rng)
        feats = make_features_from_compositions(Xj)
        qj = np.asarray(model.predict(feats), float)  # log-quantiles
        S[i] = max(0.0, float(np.min(qj)) - y_i)
    return S
...
if need_compute:
    ...
    for fam in sorted(fams):
        ...
        # train model
        model = make_quantile_estimator(tau_used, seed=fam_seed)
...
# TEST arrays (fractions + labels and high-τ preds if available)
X_test_elem = df.loc[idx_test, elem_cols].to_numpy()
y_test_mm   = np.exp(y_test)
qhat_hi_mm  = (np.exp(qhat_test_hi) if 'qhat_test_hi' in globals() else np.exp(qhat_test))

def _robust_calibrate_and_eval(alpha, eps):
    """Return coverage, median gap at (alpha, eps) on TEST."""
    # CAL side
    X_cal_elem = df.loc[idx_cal, elem_cols].to_numpy()
    if eps == 0.0:
        q_ca = (cat_qt_hi.predict(X_cal) if 'cat_qt_hi' in globals() else cat_qt.predict(X_cal))
        S = np.maximum(0.0, q_ca - y_cal)
    else:
        mdl = cat_qt_hi if 'cat_qt_hi' in globals() else cat_qt
        S = robust_scores_lower(y_cal, mdl, X_cal_elem, eps=eps, K=ROBUST_SAMPLES, rng=rng_grid)
    ...
    def _L_test(eps):
        if eps == 0.0:
            q_te = (cat_qt_hi.predict(X_test) if 'cat_qt_hi' in globals() else cat_qt.predict(X_test))
            return np.exp(q_te - q_alpha)
        mdl = cat_qt_hi if 'cat_qt_hi' in globals() else cat_qt
        L = np.zeros(len(X_test_elem))
        for i, x in enumerate(X_test_elem):
            Xj = jitter_in_L1_ball_simplex(x, eps=eps, K=ROBUST_SAMPLES, rng=rng_grid)
            feats = make_features_from_compositions(Xj)
            qj = np.asarray(mdl.predict(feats), float)
            L[i] = np.exp(np.min(qj) - q_alpha)
        return L
...
plt.tight_layout(); plt.savefig(FIGDIR/"fig_riskmap_coverage.png", dpi=300); plt.close()
...
plt.tight_layout(); plt.savefig(FIGDIR/"fig_riskmap_sharpness.png", dpi=300); plt.close()
...
# Step 12.15 — Robust lift vs novelty
from sklearn.metrics import pairwise_distances
import numpy as np, pandas as pd, matplotlib.pyplot as plt

X_all = df.loc[:, elem_cols].to_numpy(dtype=float)
X_te  = df.loc[idx_test, elem_cols].to_numpy(dtype=float)

# novelty: min L1(at.%) distance to ANY dataset composition
nov_te = pairwise_distances(X_te, X_all, metric="manhattan").min(axis=1) * 100.0
...
df_lift.to_csv(SRC_DIR/"robust_lift_vs_novelty.csv", index=False)

# Scatter + binned mean curve
plt.figure(figsize=(6.2,3.8))
plt.scatter(df_lift["novelty_atpct"], df_lift["lift_mm"], s=14, alpha=0.5)
# bins
bins = np.quantile(df_lift["novelty_atpct"], np.linspace(0,1,11))
bins = np.unique(bins)
idx  = np.digitize(df_lift["novelty_atpct"], bins[1:-1], right=True)
cent = 0.5*(bins[:-1]+bins[1:])
means = [df_lift.loc[idx==i, "lift_mm"].mean() for i in range(len(cent))]
plt.plot(cent, means, marker="o", lw=2, label="binned mean")
plt.axhline(0, ls="--", color="k", lw=1)
plt.xlabel("Novelty (min L1 distance to dataset, at.%)"); plt.ylabel("Robust lift L_robust − L_marginal (mm)")
plt.title("Robust lift grows off-manifold"); plt.legend(frameon=False)
figsave(FIGDIR/"fig_lift_vs_novelty.png")
...
df_tail = pd.DataFrame({"q80_mm": preds[0.80], "q90_mm": preds[0.90],
                        "q95_mm": preds[0.95], "q98_mm": preds[0.98]})
df_tail["tail_slope_mm"] = df_tail["q98_mm"] - df_tail["q90_mm"]
...
plt.figure(figsize=(6.2,3.8))
plt.scatter(df_tail["q98_mm"], df_tail["tail_slope_mm"], s=16, alpha=0.6)
plt.xlabel("q₀.₉₈ (mm)"); plt.ylabel("Δq (mm)")
plt.title("Heavy-tail atlas: Δq vs q₀.₉₈")
figsave(FIGDIR/"fig_tail_slope_vs_q98.png")
...
for f in fracs:
    ...
    # L_rob for TEST
    Lr = np.asarray(L_robust_mm, float)  # we could recompute; using main calibration is OK for trend
...
if 'L_robust_mm'   in globals(): methods.append(('L_robust',    L_robust_mm,    f"Robust (±{ROBUST_EPS*100:.0f} at.%)"))
...
# Family-out evaluation (enhanced, CatBoost-quantile, CIs, FDR, calibration, OOD, shift diag)
...
def binom_test_less_p(k, n, p0):
    """Exact one-sided binomial test P[X <= k | n, p0]."""
    if n <= 0: return np.nan
    ks = np.arange(0, k+1, dtype=int)
    logpmf = _log_choose(n, ks) + ks*np.log(p0) + (n-ks)*np.log(1-p0)
    m = np.max(logpmf)
    return float(np.exp(m) * np.sum(np.exp(logpmf - m)))
...
for fam_name, split in split_family_out.items():
    ...
    if _HAVE_SKOPT:
        ...
        cat_fo = BayesSearchCV(
            estimator=cb_base, search_spaces=space, n_iter=40, scoring=pin_scorer,
    ...
    for meth, L, cov, lo, hi, pv in [
        ...
        (f"Robust (±{ROBUST_EPS*100:.0f} at.%)", Lr, cov_r, lo_r, hi_r, pv_r),
    ]:
        long_rows.append({
...
_apply_fdr(df_fo, df_fo_long, "p_under_robust", f"Robust (±{ROBUST_EPS*100:.0f} at.%)")
...
# Redundancy stress (enhanced): dedup, stratified split, CP metrics, shift diagnostics, artifacts
from pathlib import Path
...
tr_big, hold = next(sss.split(np.zeros(len(df_dedup)), fam_labels))
fam_hold = fam_labels[hold]
sss2 = StratifiedShuffleSplit(n_splits=1, test_size=0.50, random_state=SEED+1)
ca, te = next(sss2.split(np.zeros(len(hold)), fam_hold))
ca = hold[ca]; te = hold[te]
tr = tr_big
...
auc = float(LogisticRegression(max_iter=1).fit(np.zeros((1,1)), [0]).classes_.shape[0])  # dummy to avoid import error
from sklearn.metrics import roc_auc_score
...
# Heaping stress (enhanced): round labels at several granularities and re-evaluate CP
import numpy as np, pandas as pd, matplotlib.pyplot as plt
from sklearn.model_selection import StratifiedShuffleSplit
...
def stress_round_and_eval(df_in, elem_cols, dmax_col, round_to_mm=0.0, use_hi_tau=True):
    ...
    sss = StratifiedShuffleSplit(n_splits=1, test_size=0.20, random_state=SEED)
    tr_big, hold = next(sss.split(np.zeros(len(df_s)), fam))
    fam_hold = fam[hold]
    sss2 = StratifiedShuffleSplit(n_splits=1, test_size=0.50, random_state=SEED+1)
    ca, te = next(sss2.split(np.zeros(len(hold)), fam_hold))
    ca = hold[ca]; te = hold[te]
...
# 13.5b — Heaping stress: plots + CSV export (enhanced + backward-compatible)
...
# Permutation sanity test (enhanced: marginal + robust, CIs, precision@k, Spearman ρ)
from pathlib import Path
...
S_ca_r = robust_scores_lower(y_perm[ca], q_model_perm, X_cal_elem, eps=ROBUST_EPS, K=ROBUST_SAMPLES, rng=rng_perm)
...
# Quantile parity & residual diagnostics (enhanced: conditional + family CIs + heteroskedasticity + PIT)
import numpy as np, pandas as pd, matplotlib.pyplot as plt
...
if len(df_fam_cal):
    plt.figure(figsize=(8.2, max(3.6, 0.28*len(df_fam_cal))))
    y = np.arange(len(df_fam_cal))
    plt.barh(y, df_fam_cal["coverage_obs"], xerr=[df_fam_cal["coverage_obs"]-df_fam_cal["ci_lo"],
                                                  df_fam_cal["ci_hi"]-df_fam_cal["coverage_obs"]],
             capsize=3, alpha=0.9)
    plt.yticks(y, [f"{f} (n={n})" for f, n in zip(df_fam_cal["family"], df_fam_cal["n"])])
    plt.axvline(tau_used, ls="--", label=f"Target τ={tau_used:.2f}")
    plt.xlabel("Observed P(Y ≤ $\\hat q_\\tau$)"); plt.xlim(0,1)
...
# --- Step 13.7b — Unified Origin/Source-data export + figure↔CSV manifest ---
...
FIGDIR  = OUTDIR / "reports" / "figures"
SRC_DIR = OUTDIR / "source_data"
for p in (REPDIR, FIGDIR, SRC_DIR): p.mkdir(parents=True, exist_ok=True)

def _exists(p): return Path(p).exists()
def _write(df, path): 
    path = Path(path); path.parent.mkdir(parents=True, exist_ok=True); df.to_csv(path, index=False); return str(path)

# ---------- Ensure we have test-time quantile predictions in scope ----------
need_q = any(v not in globals() for v in ["q_te_log","q_te_mm","y_te_log","y_te_mm","tau_used","te"])
if need_q:
    # Recreate minimal artefacts using the same split + τ
    try:
        tau_used = QT_TAU_HIGH if 'QT_TAU_HIGH' in globals() else QT_TAU
        try:
            make_quantile_estimator
        ...
        raise RuntimeError(f"Could not reconstruct test quantiles for export: {e}")

# Core exports for Origin

# 1) Parity points (test): true vs predicted high-τ quantile (mm)
df_parity = pd.DataFrame({
    "true_Dmax_mm": y_te_mm.astype(float),
    "qhat_tau_mm":  q_te_mm.astype(float),
    "tau":          float(tau_used),
    "split":        "test",
})
csv_parity = _write(df_parity, SRC_DIR / "hi_tau_quantile_parity_points.csv")

# 2) Residuals vs prediction (log): rτ = y_log − q̂τ, with predicted log on X
fam_te = df.iloc[te]["family"].values if "family" in df.columns else np.repeat("all", len(te))
df_rvp = pd.DataFrame({
    "qhat_log":        q_te_log.astype(float),
    "residual_log":    (y_te_log - q_te_log).astype(float),
    "abs_residual_log": np.abs(y_te_log - q_te_log).astype(float),
    "family":          fam_te,
    "tau":             float(tau_used),
    "split":           "test",
})
csv_rvp = _write(df_rvp, SRC_DIR / "hi_tau_quantile_residuals_vs_pred_log_points.csv")

# 3) Residuals by family (long form + summary) for box/violin in Origin
df_rbf_long = pd.DataFrame({
    "family":       fam_te,
    "residual_log": (y_te_log - q_te_log).astype(float),
    "tau":          float(tau_used),
    "split":        "test",
})
csv_rbf_long = _write(df_rbf_long, SRC_DIR / "hi_tau_quantile_residuals_by_family_long.csv")

summ = (df_rbf_long
        .groupby("family")["residual_log"]
        .agg(n="size",
             median="median",
             ...
             sd="std")
        .reset_index())
summ["tau"] = float(tau_used)
summ["split"] = "test"
csv_rbf_summ = _write(summ, SRC_DIR / "hi_tau_quantile_residuals_by_family_summary.csv")

# 4) Conditional calibration (if not already saved by 13.7)
csv_cond_cal = SRC_DIR / "quantile_conditional_calibration_bins.csv"
if not _exists(csv_cond_cal):
    hit = (y_te_log <= q_te_log).astype(int)
    qbins = np.quantile(q_te_mm, np.linspace(0, 1, 11))
    bid   = np.digitize(q_te_mm, qbins[1:-1], right=True)
    def wilson_ci_counts(k, n, z=1.96):
        if n <= 0: return (np.nan, np.nan, np.nan)
        p = k/n
        denom  = 1 + (z*z)/n
    ...
    for b in range(len(qbins)-1):
        ...
        p, lo, hi = wilson_ci_counts(k, n)
        rows.append({"bin": b,
                     "qhat_mm_lo": float(qbins[b]),
                     "qhat_mm_hi": float(qbins[b+1]),
                     "qhat_mm_center": float(0.5*(qbins[b]+qbins[b+1])),
                     "n": n, "observed_P_hit": p, "ci_lo": lo, "ci_hi": hi,
                     "target_tau": float(tau_used)})
    _write(pd.DataFrame(rows), csv_cond_cal)

# 5) Family-wise calibration (if not already saved by 13.7)
csv_fam_cal = SRC_DIR / "quantile_family_calibration_wilson.csv"
if not _exists(csv_fam_cal):
    hit = (y_te_log <= q_te_log).astype(int)
    def wilson_ci_counts(k, n, z=1.96):
        if n <= 0: return (np.nan, np.nan, np.nan)
        p = k/n
        ...
        return float(p), max(0.0, center - half), min(1.0, center + half)
    rows = []
    fam_series = pd.Series(range(len(te)), index=fam_te).groupby(level=0)
    for fam, idx in fam_series:
        ii = idx.values
        if len(ii) < 5: 
            continue
        k, n = int(hit[ii].sum()), int(len(ii))
        p, lo, hi = wilson_ci_counts(k, n)
        rows.append({"family": fam, "n": n, "coverage_obs": p, "ci_lo": lo, "ci_hi": hi, "target_tau": float(tau_used)})
    _write(pd.DataFrame(rows).sort_values(["n","family"], ascending=[False, True]), csv_fam_cal)

# 6) Heteroskedasticity running-median |residual| (if missing)
csv_absres = SRC_DIR / "quantile_residual_running_median.csv"
if not _exists(csv_absres):
    qbins = np.quantile(q_te_log, np.linspace(0, 1, 16))
    bid   = np.digitize(q_te_log, qbins[1:-1], right=True)
    med_x, med_absr, ns = [], [], []
    abs_r = np.abs(y_te_log - q_te_log)
    for b in range(len(qbins)-1):
        m = (bid == b)
        if not np.any(m): continue
        med_x.append(float(np.median(q_te_log[m])))
        med_absr.append(float(np.median(abs_r[m])))
        ns.append(int(m.sum()))
    _write(pd.DataFrame({"qhat_log_bin_center": med_x, "median_abs_residual": med_absr, "n_in_bin": ns}), csv_absres)

# 7) Optional PIT-style curve across τ (only if multi-τ models exist and CSV not present)
csv_pit = SRC_DIR / "quantile_pit_curve.csv"
if (not _exists(csv_pit)) and ('cat_qt_grid' in globals() and isinstance(cat_qt_grid, dict) and len(cat_qt_grid) >= 3):
    rows = []
    for t in sorted(cat_qt_grid.keys()):
        q_t_log = np.asarray(cat_qt_grid[t].predict(X.iloc[te]), float)
        hit_t   = (y_te_log <= q_t_log).astype(int)
        k, n = int(hit_t.sum()), int(len(hit_t))
        # Wilson
        ...
        center = (p + z*z/(2*n))/denom
        half   = (z/denom)*np.sqrt((p*(1-p)/n)+z*z/(4*n*n))
        rows.append({"tau": float(t), "observed": float(p),
                     "ci_lo": float(max(0.0, center-half)), "ci_hi": float(min(1.0, center+half))})
    _write(pd.DataFrame(rows).sort_values("tau"), csv_pit)

# ---------- Build a manifest mapping FIGURES → CSV(s) we’ve produced ----------
pairs = []

# Add explicit pairs for the diagnostics in this step
pairs += [
    (FIGDIR / "fig_quantile_parity_mm.png",                 [csv_parity]),
    (FIGDIR / "fig_quantile_residual_vs_pred_log.png",      [csv_rvp]),
    (FIGDIR / "fig_quantile_absres_vs_pred_log.png",        [csv_absres]),
    (FIGDIR / "fig_quantile_conditional_calibration.png",   [csv_cond_cal]),
    (FIGDIR / "fig_quantile_calibration_by_family.png",     [csv_fam_cal]),
    (FIGDIR / "fig_quantile_pit_curve.png",                 [csv_pit]),
]

# Add pairs for earlier figures if they and their CSVs exist
_optional = [
    (FIGDIR / "fig_reliability_coverage.png",           SRC_DIR / "hi_tau_coverage_bars.csv"),
    (FIGDIR / "fig_reliability_sharpness.png",          SRC_DIR / "hi_tau_sharpness_gaps.csv"),
    (FIGDIR / "fig_cov_by_true_bin_marg_robust.png",    SRC_DIR / "cov_by_true_bin.csv"),
    (FIGDIR / "fig_L_vs_true_scatter.png",              SRC_DIR / "L_vs_true_points.csv"),
    (FIGDIR / "fig_shift_weight_hist.png",              SRC_DIR / "shift_weights_cal.csv"),
    (FIGDIR / "fig_worstcase_drop_hist.png",            SRC_DIR / "worstcase_drop_by_eps.csv"),
    (FIGDIR / "fig_learning_curve_coverage.png",        SRC_DIR / "learning_curve_metrics.csv"),
    (FIGDIR / "fig_learning_curve_precision15.png",     SRC_DIR / "learning_curve_metrics.csv"),
    (FIGDIR / "fig_coverage_by_ood.png",                SRC_DIR / "coverage_by_ood.csv"),
    (FIGDIR / "fig_ablation_familyout_coverage.png",    SRC_DIR / "ablation_familyout.csv"),
    (FIGDIR / "fig_heaping_coverage_vs_round.png",      SRC_DIR / "heaping_stress_summary.csv"),
    (FIGDIR / "fig_heaping_sharpness_vs_round.png",     SRC_DIR / "heaping_stress_summary.csv"),
    (FIGDIR / "fig_family_out_per_family_coverage_robust_wilson.png",
                                                      SRC_DIR / "family_out_per_family_coverage_wilson_all_methods.csv"),
    (FIGDIR / "fig_family_out_summary_coverage_bootstrap.png",
                                                      SRC_DIR / "family_out_summary_bootstrap_means.csv"),
    (FIGDIR / "fig_family_out_summary_sharpness_bootstrap.png",
                                                      SRC_DIR / "family_out_summary_bootstrap_means.csv"),
]
for fig_p, csv_p in _optional:
    if _exists(fig_p) and _exists(csv_p):
        pairs.append((fig_p, [csv_p]))

# Precision@k figures (threshold-specific)
if 'THRESHOLDS_MM' in globals():
    for D in THRESHOLDS_MM:
        fig_p = FIGDIR / f"fig_precision_k_{int(D)}mm.png"
        csv_p = SRC_DIR / f"precision_at_k_{int(D)}mm.csv"
        if _exists(fig_p) and _exists(csv_p):
            pairs.append((fig_p, [csv_p]))

# Design boards (threshold-specific shortlists)
if 'BOARD_THRESHOLDS' in globals():
    for D in BOARD_THRESHOLDS:
        fig_p = FIGDIR / f"fig_design_board_{int(D)}mm.png"
        csv_p = OUTDIR / "data" / "designed" / f"design_shortlist_{int(D)}mm.csv"
        if _exists(fig_p) and _exists(csv_p):
            pairs.append((fig_p, [csv_p]))

# Build and save manifest
manifest = []
for fig_p, csv_list in pairs:
    manifest.append({"figure": str(fig_p), "csv": [str(c) for c in csv_list]})
with open(REPDIR / "origin_export_manifest.json", "w") as f:
    json.dump({"figure_to_csv": manifest}, f, indent=2)

print("Origin/source-data exports written:")
print(" -", csv_parity)
print(" -", csv_rvp)
print(" -", csv_rbf_long)
print(" -", csv_rbf_summ)
print(" -", csv_cond_cal if _exists(csv_cond_cal) else "(no conditional-calibration CSV)")
print(" -", csv_fam_cal if _exists(csv_fam_cal) else "(no family-calibration CSV)")
print(" -", csv_absres if _exists(csv_absres) else "(no abs-residuals CSV)")
print(" -", csv_pit if _exists(csv_pit) else "(no PIT CSV)")
print("Manifest:", REPDIR / "origin_export_manifest.json")


# In[ ]:


# 13.8 — Export compact SI tables
df_perfam.to_csv(OUTDIR/"reports"/"per_family_coverage_random.csv", index=False)
if 'df_fo' in globals():
    df_fo.to_csv(OUTDIR/"reports"/"family_out_metrics.csv", index=False)
print("Saved SI tables in", OUTDIR/"reports")
